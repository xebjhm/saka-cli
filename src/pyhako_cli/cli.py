...
def detect_system_language():
    ...
    try:
        ...
        # Priority 2: locale module (OS Default)
        if not lang_sys:
            try:
                # locale.getdefaultlocale() is standard for detecting system default
                # despite deprecation warnings in newer Python, it's reliable for CLI tools
                lang_sys = locale.getdefaultlocale()[0]
            except:
                pass
...
class HakoCLI:
    ...
    def run_interactive_wizard(self):
        ...
        # -1. Language Selection (First time override)
